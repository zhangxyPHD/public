...
#SBATCH --job-name=Oh_OhVALUE_We_WeVALUE_INDEX-2
...
set_threads_by_maxlevel() {
  case $1 in
    ...
    11) echo 24 ;; # MAXlevel = 10 -> THREADS = 5
    12) echo 24 ;; # MAXlevel = 10 -> THREADS = 5
    13) echo 24 ;; # MAXlevel = 10 -> THREADS = 5
    *) echo 1 ;;  # Default
  esac
}
          ...
          for epsilon in "${epsilons[@]}"; do
            ...
            (
              ...
              while true; do
                ...
                mpirun -n $THREADS ./bounce "$MAXlevel" "$MAXlevel1" "$J" "$We" "$Oh" "$Bo" "$epsilon" "$tmax" "$Ldomain" "1e-4" "0.1" "$FILENAME" >log_error 2>&1
                ...
                if grep -q "Assertion" log_error || grep -q "Aborted" log_error || grep -q "Ke_Error" log_error; then
                  ...
                  mpirun -n $THREADS ./bounce "$MAXlevel" "$MAXlevel1" "$J" "$We" "$Oh" "$Bo" "$epsilon" "$tmax" "$Ldomain" "1e-5" "0.1" "$FILENAME" >log_error1 2>&1
                  ...
                  if grep -q "Assertion" log_error1 || grep -q "Aborted" log_error1 || grep -q "Ke_Error" log_error1; then
                    rm -f "../Results_Running/log_$FILENAME"
                    mpirun -n $THREADS ./bounce "$MAXlevel" "$MAXlevel1" "$J" "$We" "$Oh" "$Bo" "$epsilon" "$tmax" "$Ldomain" "1e-6" "0.1" "$FILENAME" >log_error2 2>&1
